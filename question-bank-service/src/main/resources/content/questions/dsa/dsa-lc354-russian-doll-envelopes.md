...
- lc-354
buildProfile: judge0
source: inspired-by:operator-corpus/Q_128_Russian_Doll_Envelopes.java
status: PUBLISHED
sampleTests:
- name: Sample 1
  input: '3 3
...
editorial: '### Russian Doll Envelopes
...
solutionCode: "import java.util.*;\n\nimport java.util.Arrays;\n\npublic class Main {\n    \n    public static int  maxEnvelopes(int[][] envelopes){\n        if(envelopes==null || envelopes.length==0){return 0;}\n        Arrays.sort(\n            envelopes,(a,b) -> { if(a[0] !=b[0]) return a[0]-b[0];\n                return b[1]-a[1];});\n    \n                int[] tails=new int[envelopes.length];\n                int len=0;\n\n                for(int[] env : envelopes){\n                    int h=env[1];\n                    int index=Arrays.binarySearch(tails,0,len,h);\n                    if(index<0){\n                        index=-(index+1);\n                    }\n                    tails[index]=h;\n                    if(index==len) len++;\n                }\n                return len;\n            }            \n\n    public static void main(String[] args) {\n        Scanner sc = new Scanner(System.in);\n        if (!sc.hasNext()) return;\n        int r = sc.nextInt();\n     \
  \   int c = sc.nextInt();\n        int[][] mat = new int[r][c];\n        for (int i = 0; i < r; i++) for (int j = 0; j < c; j++) mat[i][j] = sc.nextInt();\n        System.out.println(maxEnvelopes(mat));\n    }\n}"
---
