class IsolateJob < ApplicationJob
  ...
  def extract_archive
    return unless submission.is_project

    archive = boxdir + "/" + ADDITIONAL_FILES_ARCHIVE_FILE_NAME
    File.open(archive, "wb") { |f| f.write(submission.additional_files) }
    ...
    #{cgroups.present? ? "--cg-mem=#{Config::MAX_MEMORY_LIMIT}" : (submission.enable_per_process_and_thread_memory_limit ? "-m #{Config::MAX_MEMORY_LIMIT}" : "")} \
  ...
  def compile
    ...
    mem_flag = ""
    if cgroups.present?
      mem_flag = "--cg-mem=#{Config::MAX_MEMORY_LIMIT}"
    elsif submission.enable_per_process_and_thread_memory_limit && !is_jvm
      mem_flag = "-m #{Config::MAX_MEMORY_LIMIT}"
    end
  ...
  def run
    ...
    mem_flag = ""
    if cgroups.present?
      mem_flag = "--cg-mem=#{submission.memory_limit}"
    elsif submission.enable_per_process_and_thread_memory_limit && !is_jvm
      mem_flag = "-m #{submission.memory_limit}"
    end
