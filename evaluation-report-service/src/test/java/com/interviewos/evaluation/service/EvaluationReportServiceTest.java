...
class EvaluationReportServiceTest {
    ...
    void testExecutionScoreWithCompileErrorOnly() {
        List<SessionServiceClient.TranscriptMessageDto> transcript = List.of(
                new SessionServiceClient.TranscriptMessageDto(1L, "CANDIDATE", "CODE_EXECUTION", "Candidate executed code: 0/4 tests passed (COMPILE_ERROR) in 0.0ms.", "class Main {}", Instant.now()),
    ...
    void testExecutionScoreWithTimeout() {
        List<SessionServiceClient.TranscriptMessageDto> transcript = List.of(
                new SessionServiceClient.TranscriptMessageDto(1L, "CANDIDATE", "CODE_EXECUTION", "Candidate executed code: 2/4 tests passed (TIMEOUT) in 5000.0ms.", "class Main {}", Instant.now()),
    ...
    @Test
    @DisplayName("4/4 Passing execution + LLM Rubric -> Verified HIRE with full 5 dimensions and verbatim quotes")
    void testPassingExecutionWithLlmRubric() {
        List<SessionServiceClient.TranscriptMessageDto> transcript = List.of(
                ...
                new SessionServiceClient.TranscriptMessageDto(2L, "CANDIDATE", "CODE_EXECUTION", "Candidate executed code: 4/4 tests passed (PASSED) in 120.0ms.", "class Main {}", Instant.now())
        );
        when(sessionClient.getSessionTranscript(1L)).thenReturn(transcript);
